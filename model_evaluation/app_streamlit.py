...
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'labs', 'get_data'))
...
from labs.get_data.generate_eth_features_history import (
...
def cargar_dataset_mas_reciente():
    ...
    if not archivos:
        st.error("No se encontró ningún archivo que coincida con el patrón 'features_recent_*_*.csv' en el directorio labs/get_data")
        return None
...
def descargar_datos_recientes(minutes, max_tx):
    ...
    with st.spinner("Descargando transacciones recientes..."):
        try:
            ...
            features.to_csv(os.path.join("../labs/get_data", filename), index=False)
